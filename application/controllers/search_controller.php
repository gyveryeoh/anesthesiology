  ...
  function searchcaselog_details()
  {
   if($this->session->userdata('logged_in'))
   {
     ...
     $data['id'] = $session_data['id'];
          ...
          {
          $config["total_rows"] = $this->search_caselogs->count_search_caselog_details_1($user_id);
          }
          if ($status_id != 0 && $user_id == 0)
          {
           $config["total_rows"] = $this->search_caselogs->count_search_caselog_details_2($status_id);
          }
          if ($status_id == 0 && $user_id == 0)
          {
           $config["total_rows"] = $this->search_caselogs->count_search_caselog_details_3();
          }
          if ($status_id != 0 && $user_id != 0)
          {
           $config["total_rows"] = $this->search_caselogs->count_search_caselog_details_4($status_id,$user_id);
          }
          $config["per_page"] = 10;
